...
from urban_planning.models.model import create_rgnn_model, create_sgnn_model, create_mlp_model, ActorCritic
...
class UrbanPlanningAgent(AgentPPO):
    ...
    def setup_model(self):
        cfg = self.cfg
        if cfg.agent == 'rl-rgnn':
            self.policy_net, self.value_net, self.reward_net = create_rgnn_model(cfg, self)
            self.actor_critic_net = ActorCritic(self.policy_net, self.value_net)
            to_device(self.device, self.actor_critic_net)
        elif cfg.agent == 'rl-sgnn':
    ...
    def optimize_policy(self, iteration):
        ...
        """evaluate policy"""
        for i in range(19):
            log_eval = self.eval_agent(num_samples=1, mean_action=False)
