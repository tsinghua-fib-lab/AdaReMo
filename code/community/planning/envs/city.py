...
def reward_info_function(
    ...
    if name == 'intermediate':
        ...
        trace_reward = plc.get_trace_reward()
        proxy_reward += trace_reward
...
class CityEnv:
    ...
    def save_gdf(self) -> None:
        """
        Save the gdf.
        """
        gdf = self._plc.get_gdf()
        files_number = int(len(os.listdir('./save_gdf/raw')))
        pickle.dump(gdf, open(f'./save_gdf/raw/{files_number}.pkl', 'wb'))
