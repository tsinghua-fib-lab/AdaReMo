...
class PlanClient(object):
    ...
    def init_residents(self) -> None:
        self.resident_trajectories = pickle.load(open('1k_7_activity.pkl', 'rb'))
        gdf_demo = pickle.load(open('./save_gdf/old/0.pkl', 'rb'))
        gdf_demo_residential = gdf_demo[(gdf_demo['existence'] == True) & (gdf_demo['type'] == 4)]
        residents_home = gdf_demo_residential.sample(n = self.residents_number, weights = gdf_demo_residential.area, replace=True)
        self.home_index = residents_home.index.to_list()
        self.residents_rng = np.random.default_rng()
    ...
    def get_trace_reward(self, quick=False, alg='greedy', seed=0):
        gdf = self._gdf
        total_travel_distance = []
        self.residents_rng.seed(seed)
        if quick:
            residents_number = self.residents_number_quick
            resident_trajectories = self.resident_trajectories_quick
        else:
            residents_number = self.residents_number
            resident_trajectories = self.resident_trajectories
            
        if alg == 'greedy':
            nearest_distances = {}
            centroids = np.stack([gdf.centroid.x, gdf.centroid.y], axis=1)
            all_types = gdf['type'].unique()
            for i in range(len(gdf)):
                current_point_coordinates = centroids[i]
                target_dict = {}
                for target_type in all_types:
                    target_gdf = gdf[gdf['type'] == target_type]
                    target_coordinates = np.stack([target_gdf.centroid.x, target_gdf.centroid.y], axis=1)
                    distances = cdist([current_point_coordinates], target_coordinates)[0]
                    min_distance = np.min(distances)
                    min_distance_index = target_gdf.index[np.argmin(distances)]
                    target_dict[target_type] = (min_distance_index, min_distance)
                nearest_distances[gdf.index[i]] = target_dict
            
            for i in range(residents_number):
                resident_travel_distance = 0
                start_index = self.home_index[i]
                start = gdf.loc[start_index]
                current_index = start_index
                for next_type in resident_trajectories[i][1:-1]:
                    if next_type == city_config.RESIDENTIAL:
                        current_poi = gdf.loc[current_index]
                        resident_travel_distance += cdist([[start.geometry.centroid.x, start.geometry.centroid.y]], [[current_poi.geometry.centroid.x, current_poi.geometry.centroid.y]])[0][0]
                        current_index = start_index
                    else:
                        nearest_info = nearest_distances[current_index][next_type]
                        resident_travel_distance += nearest_info[1]
                        current_index = nearest_info[0]
                
                last_poi = gdf.loc[current_index]
                resident_travel_distance += cdist([[start.geometry.centroid.x, start.geometry.centroid.y]], [[last_poi.geometry.centroid.x, last_poi.geometry.centroid.y]])[0][0]
                total_travel_distance.append(resident_travel_distance)
        ...
        if alg == 'gravity':
            ...
            for i in range(len(gdf)):
                current_point_coordinates = centroids[i]
                target_dict = {}
                for target_type in gdf['type'].unique():
                    target_gdf = gdf[gdf['type'] == target_type]
                    target_coordinates = np.stack([target_gdf.centroid.x, target_gdf.centroid.y], axis=1)
                    target_distances = cdist([current_point_coordinates], target_coordinates)[0]
                    probs = target_gdf.area / ((target_distances + 1e-6)**2.5)
                    probs /= probs.sum()
                    target_dict[target_type] = np.stack([target_gdf.index, probs, target_distances], axis=1)
                visit_probs[gdf.index[i]] = target_dict
            ...
            for i in range(residents_number):
                ...
                for next_type in resident_trajectories[i][1:-1]:
                    if next_type == city_config.RESIDENTIAL:
                        current_poi = gdf.loc[current_index]
                        target_distances = cdist([[current_poi.geometry.centroid.x, current_poi.geometry.centroid.y]], [[start.geometry.centroid.x, start.geometry.centroid.y]])[0][0]
                        resident_travel_distance += target_distances
                        current_index = start_index
                    else:
                        target_dict = visit_probs[current_index][next_type]
                        next_index = self.reisdents_rng.choice(target_dict[:, 0], p=target_dict[:, 1])
                        next_index_list = target_dict[:, 0].tolist().index(next_index)
                        resident_travel_distance += target_dict[:, 2][next_index_list]
        ...
        trace_reward = -np.sum(total_travel_distance) / (residents_number * self.days_number) / 1e3 + 2
        
        return trace_reward, dict()